...
| User Agents Appendix                | Optional `AGENTS.md` file under `SHRIMP_STATE_DIR`; when present, its content is appended to the System Prompt of every Job (both HeartbeatJob and ChannelJob)          |
...
- **User Agents Appendix read failure**: if `SHRIMP_STATE_DIR/AGENTS.md` is absent, the System Prompt is assembled without an appendix (this is the expected default, not an error). If the file exists but cannot be read (permission denied, I/O error, etc.), the failure is logged at warn with the path and error code, and the Job proceeds with the System Prompt unchanged — the Job is never failed because of an AGENTS.md read error.
...
- **User Agents Appendix:** At each Job execution (both HeartbeatJob and ChannelJob), Shrimp attempts to read `AGENTS.md` from `SHRIMP_STATE_DIR`. If the file exists and is readable, its trimmed content is appended to the System Prompt as a final section, after the base, variant, and tool sections. If the file is absent, the System Prompt is unchanged — this is the expected default and is not treated as an error. The Appendix never replaces any built-in section and has no defined schema; operators are responsible for its content. Read errors are handled per [Failure Handling](#failure-handling).
...
| `SHRIMP_STATE_DIR`             | Filesystem directory holding Session state (`state.json` and `sessions/<id>.jsonl` files) when Channels are enabled, and the optional `AGENTS.md` file (User Agents Appendix) read on every Job.                                                                    | No (default: `~/.shrimp`)                                                              |
...
- **`SHRIMP_STATE_DIR` is read only when Channels are enabled** for Session persistence; the directory is created at startup if missing and the process fails fast if creation fails. The User Agents Appendix (`AGENTS.md`) is also resolved under this directory and is read on every Job regardless of whether Channels are enabled — when Channels are off, no directory creation is performed and a missing directory simply results in no Appendix.
- **User Agents Appendix is optional.** Shrimp reads `AGENTS.md` from `SHRIMP_STATE_DIR` on every Job (see [Job § Prompt rules](#job)); a missing file is the expected default. See [Failure Handling](#failure-handling) for read error behavior.
