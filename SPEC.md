...
| 3    | Job Worker      | Load Session                  | Read the current Session via the Session store, or create a new Session if this is the first message; see [Session Lifecycle](#session-lifecycle)                                        |
...
| OTel mapping | `gen_ai.conversation.id` (OTel GenAI semconv). This is a per-invocation correlation key: each Job represents a single, discrete Shrimp Agent invocation, and the Job ID groups all spans emitted within it. Cross-Job conversation correlation (for ChannelJobs backed by a Session) is provided by Session ID — the Shrimp Agent receives history as input rather than holding it as state. The Job ID satisfies the "group all spans for one invocation" use case that `gen_ai.conversation.id` is designed for |
