...
| Skill Catalog        | The set of `(name, description, absolute SKILL.md path)` triples assembled from all valid discovered skills and embedded in the System Prompt in place of per-tool capability paragraphs                                                                                                                                                                                                                                                          |
...
| ------------- | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| System prompt | Dynamic, assembled per execution | Goal setting (complete the task, report progress) + Skill Catalog (for each discovered skill: name, description, absolute SKILL.md path) | Same structure; goal framing reflects conversational context                                            |
...
- The system prompt is assembled at each Job execution. It describes the Shrimp Agent's goal and carries the Skill Catalog — one entry per discovered skill with its name, description, and absolute SKILL.md path (see [Skill Layer](#skill-layer)) — so the model knows which skills are available and can request each skill's full instructions on demand. Tool definitions for function calling are provided separately via AI SDK's tools parameter; the system prompt provides the human-readable context that guides tool usage.
