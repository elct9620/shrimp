...
| Heartbeat ID       | A UUID generated by the Processing Cycle at the start of each cycle; used as a domain-scoped correlation key to group all spans and logs produced by that cycle. Specifically the source of `gen_ai.conversation.id` on the `shrimp.main-agent` span. Not a session identifier, not persisted, and not correlated across cycles or restarts.         |
