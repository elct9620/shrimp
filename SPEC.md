...
| 5a   | Job Worker      | Deliver replies               | After the agent returns, delivers the agent's final assistant ConversationMessage (at most one per invocation, by design) to the originating Channel conversation via ChannelGateway using the ConversationRef; failures are Fail-Open                          |
...
| JSONL append failure during a Job                                                                                      | Fail-Open Recovery — the Job is not failed solely because a conversation entry could not be persisted. The user and assistant appends occur independently (user before the agent runs, assistant after) and each is Fail-Open; a failure on one does not roll back the other, so the Session may retain an unpaired user turn OR an unpaired assistant turn. Either asymmetry is tolerated; the loss is logged and the next message continues from the last successfully persisted entry. |
...
| 3a   | Job Worker   | Deliver the agent's final assistant ConversationMessage (at most one per invocation, by design) to the originating Channel conversation via ChannelGateway; failures are Fail-Open |
...
| ------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| System prompt | Dynamic, assembled per execution | Goal-setting and workflow framing (skill-first approach, working-style norms) + any variant-specific directives + Skill Catalog (for each discovered skill: name, description) + `## Tools` section (progressive-disclosure usage of `skill` / `read`) | Same base structure + an `## Output Format` section appended after `## Tools`, constraining the form of the agent's final assistant text for delivery to the Channel |
...
- **Output Format (ChannelJob only):** The System Prompt for a ChannelJob carries a named `## Output Format` section that constrains the form of the final assistant text the Shrimp Agent delivers to the Channel (see [Shrimp Agent Role contract Output](#shrimp-agent)). It instructs the agent to reply in plain text suited to the destination Channel — no Markdown or HTML rendering is performed by the Channel, so markup symbols (hash signs, asterisks, backticks, hyphen-bullets, numbered lists, blockquotes, bracket-parenthesis link syntax) appear as literal characters and are disallowed; multiple items MUST be woven into running prose rather than listed. The section lands after the base, variant directives, Skill Catalog, and `## Tools` section, and BEFORE the User Agents Appendix. HeartbeatJob System Prompts do NOT carry this section — HeartbeatJob outcomes are surfaced through Todoist tool calls (comment posts, section moves), not as human-facing free-text replies.
- **User Agents Appendix:** At each System Prompt assembly (exactly once per Job, since each Job assembles exactly one System Prompt), Shrimp attempts to read `AGENTS.md` from `SHRIMP_HOME`. If the file exists and is readable, its trimmed content is appended to the System Prompt as the final section, after the base, variant, Skill Catalog, `## Tools`, and — for ChannelJob only — `## Output Format` sections. **No content MUST follow the User Agents Appendix.** Any new prompt content introduced in the future MUST land before this section. If the file is absent, the System Prompt is unchanged — this is the expected default and is not treated as an error. The Appendix never replaces any built-in section and has no defined schema; operators are responsible for its content. Shrimp performs no length enforcement, truncation, or validation — oversize content that exceeds the model's context budget is treated as a normal AI provider error (handled by the Shrimp Agent's error path, not by this feature). Read errors are handled per [Failure Handling](#failure-handling).
...
| Output        | Loop terminated. By design, the agent surfaces at most one assistant ConversationMessage — its final output text — regardless of how many internal steps ran; intermediate reasoning and tool-use turns never leave the agent. For a HeartbeatJob the agent also performs its Todoist work (progress comments, moving the task to Done) via tool calls during the loop; for a ChannelJob the final assistant ConversationMessage is what the Job Worker delivers to the Channel (at most one reply per ChannelJob) |
...
| `AI_REASONING_EFFORT`             | Reasoning-effort hint forwarded to the AI provider for models that support it. Treated as an opaque string passed through unchanged; empty string is treated as unset.                                                                                                                      | No (default: unset — no reasoning-effort hint sent)                                    |
...
| `LOG_LEVEL`                       | Minimum severity emitted by the structured logger. Allowed values: `trace`, `debug`, `info`, `warn`, `error`, `fatal`, `silent`. Any other value causes the process to fail fast at startup.                                                                                                | No (default: `info`)                                                                   |
