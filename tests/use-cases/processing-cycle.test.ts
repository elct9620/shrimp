...
type RecordingTelemetry = TelemetryPort & {
  runInSpan: ReturnType<typeof vi.fn>;
};
...
describe("ProcessingCycle.run", () => {
  ...
  describe("when an In Progress task is selected", () => {
    ...
    it("should include comment text in user prompt", async () => {
      board.getComments = vi
        .fn()
        .mockResolvedValue([makeComment("prior progress note")]);
  ...
  describe("when BoardSectionMissingError is thrown", () => {
    it("should end the cycle cleanly without calling main agent", async () => {
      board.getTasks = vi
        .fn()
        .mockRejectedValue(new BoardSectionMissingError("In Progress"));
    ...
    it("should not throw", async () => {
      board.getTasks = vi
        .fn()
        .mockRejectedValue(new BoardSectionMissingError("Backlog"));
  ...
  describe("PromptAssembler receives correct inputs", () => {
    it("should pass tool descriptions from ToolProvider to prompt assembler (visible in system prompt)", async () => {
      ...
      toolProvider.getToolDescriptions = vi
        .fn()
        .mockReturnValue([
  ...
  describe("logging", () => {
    ...
    it('should log warn "cycle skipped" with missingSection when BoardSectionMissingError is thrown', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          missingSection: expect.stringContaining("In Progress"),
        }),
      );
    });
  ...
  describe("telemetry span wrapping", () => {
    ...
    it("should let BoardSectionMissingError resolve normally through runInSpan", async () => {
      const telemetry = makeFakeTelemetry();
      board.validateSections = vi
        .fn()
        .mockRejectedValue(new BoardSectionMissingError("Done"));
      ...
      await expect(localCycle.run()).resolves.toBeUndefined();
    });

    it("should propagate unexpected errors out of runInSpan", async () => {
      const telemetry = makeFakeTelemetry();
      ...
      await expect(localCycle.run()).rejects.toBe(boom);
    });
  });
});
