...
describe('assemble', () => {
  describe('system prompt', () => {
    ...
    it('lists each tool name in the system prompt', () => {
      ...
      const { systemPrompt } = assemble({ task: makeTask(), comments: [], tools })
    ...
    it('preserves tool description input order in the system prompt', () => {
      ...
      expect(systemPrompt.indexOf('alpha_tool')).toBeLessThan(systemPrompt.indexOf('beta_tool'))
    })

    it('produces the same output for the same input (pure function)', () => {
      const input = { task: makeTask(), comments: [], tools: makeTools(['t', 'd']) }
