...
describe('createPostCommentTool', () => {
  ...
  it('should accept valid taskId and text input', () => {
    const schema = createPostCommentTool(repo, makeFakeLogger()).inputSchema as unknown as ParseableSchema
  ...
  describe('logging', () => {
    ...
    it('should log warn and rethrow when repo throws', async () => {
      ...
      await expect(
        t.execute!({ taskId: 'task-42', text: 'x' }, { toolCallId: 'test', messages: [] })
