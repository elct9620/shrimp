...
describe('createMoveTaskTool', () => {
  ...
  it('should accept valid taskId and Backlog section', () => {
    const schema = createMoveTaskTool(repo, makeFakeLogger()).inputSchema as unknown as ParseableSchema
  ...
  describe('logging', () => {
    it('should log debug on invocation with taskId and section', async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        'tool invoked',
        expect.objectContaining({ input: { taskId: 'task-42', section: 'Done' } }),
      )
    })
    ...
    it('should log warn and rethrow when repo throws', async () => {
      ...
      await expect(
        t.execute!({ taskId: 'task-42', section: 'Done' }, { toolCallId: 'test', messages: [] })
