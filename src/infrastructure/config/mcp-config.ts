...
export function parseMcpConfig(raw: string): McpConfig {
  ...
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new McpConfigError('Invalid .mcp.json: root value must be a JSON object')
  }

  const root = parsed as Record<string, unknown>

  if (!('mcpServers' in root)) {
    throw new McpConfigError('Invalid .mcp.json: missing required key "mcpServers"')
  }

  const mcpServers = root['mcpServers']

  if (mcpServers === null || typeof mcpServers !== 'object' || Array.isArray(mcpServers)) {
    throw new McpConfigError('Invalid .mcp.json: "mcpServers" must be a JSON object')
  }
