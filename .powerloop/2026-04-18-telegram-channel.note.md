...
| 6 | Refactor Job: split into abstract Job Worker skeleton + HeartbeatJob (current behavior, empty history) + ChannelJob (load Session → assemble prompt with incoming msg → invoke Agent with history → append new ConversationMessage entries); extend JobInput with history: ConversationMessage[]; HeartbeatRoute wires HeartbeatJob | in_progress | pending | pending | SPEC §Job; JobQueue stays variant-agnostic |
| 7 | Use case: StartNewSession — archives previous Session, creates new, updates state.json; bypasses JobQueue; used by /new | pending | pending | pending | SPEC §Slash Commands, §Session Lifecycle |
