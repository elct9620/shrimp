...
| 8 | Adapter (tool): ReplyTool under adapters/tools/built-in/ — calls ChannelGateway with ConversationRef; no-op when ref is null (HeartbeatJob path) | in_progress | pending | pending | SPEC §Channel Integration dispatch; architecture.md row 88 |
...
| 8 | execute | Item 7 complete. StartNewSession use case: thin class delegating to SessionRepository.createNew(); one info log; returns new Session. No tests — no branching, and the full path is integration-tested by JsonlSessionRepository + to-be item-10 webhook test. 430/430 pass. Commit 96db748. | Method named `execute()` (generic use-case verb) to distinguish from the Job Worker `run()` signature, since StartNewSession bypasses the Job Queue per SPEC §Slash Commands. | Next cycle: item 8 (ReplyTool). Built-in tool under src/adapters/tools/built-in/reply.ts. AI SDK tool schema with `message: string` parameter. Calls ChannelGateway.reply(ref, text). No-op when ref is null (HeartbeatJob path — architecture.md explicitly says "no-op when ConversationRef is null"). Needs per-job ref injection — check existing built-in-tool-factory.ts pattern: tools likely receive BoardRepository via factory; ReplyTool will need the current Job's ConversationRef, so think about whether the factory accepts a per-invocation context or whether we pass the ref as a separate field on the tool factory's input. Keep SPEC-Comment-Tag style consistency: for Todoist's post-comment, the tag prefix is added at tool boundary; ReplyTool has no such prefix. Integration test: tool executes → ChannelGateway.reply called with expected ref and text; null-ref case returns something "gracefully no-op'd" (return a short success string so AI SDK doesn't interpret as error). |
