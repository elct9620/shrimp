...
| **Use Cases**            | Application orchestration: `Job` (the orchestrator that selects a task, prepares prompt context, and invokes the Shrimp Agent once per heartbeat), `PromptAssembler`, and the **port interfaces** for every external dependency (`BoardRepository`, `ShrimpAgent`, `TaskQueue`, `ToolProvider`). This layer has no knowledge of Hono, AI SDK, the Todoist client, or MCP implementations.                                                                          | Entities                                 |
...
| HTTP Layer               | `HeartbeatRoute`                                                             | Interface Adapter | Accepts `POST /heartbeat`, delegates to `TaskQueue`, responds `202 Accepted`                                                                                                                                                                                                                                                                                                                                                                   | `TaskQueue`, `Job`                                                                                             |
| Supervisor               | (implemented across the HTTP layer + DI container; no single dedicated file) | Composition Root  | Internal component that receives heartbeats, owns the Job Queue, and dispatches Job Workers; realized by `HeartbeatRoute`, `InMemoryTaskQueue`, and the DI wiring in `container.ts`                                                                                                                                                                                                                                                            | `TaskQueue`, `Job`                                                                                             |
| Job Queue                | `InMemoryTaskQueue`                                                          | Infrastructure    | Single-slot concurrency gate implementation; bracket pattern guarantees slot release                                                                                                                                                                                                                                                                                                                                                           | —                                                                                                              |
| Job Worker               | `Job`                                                                        | Use Case          | Orchestrates one heartbeat-triggered unit of work. Selects a task via `TaskSelector` and `BoardRepository`, promotes Backlog→InProgress, assembles prompts via `PromptAssembler`, and invokes `ShrimpAgent` **once**. Everything the AI does during its loop happens inside that single invocation.                                                                                                                                            | `BoardRepository`, `ShrimpAgent`, `ToolProvider`, `TaskSelector`, `PromptAssembler`                            |
