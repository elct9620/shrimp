...
| **Use Cases**            | Application orchestration: `Job` (the orchestrator that selects a task, prepares prompt context, and invokes the Shrimp Agent once per heartbeat), `PromptAssembler`, and the **port interfaces** for every external dependency (`BoardRepository`, `ShrimpAgent`, `JobQueue`, `ToolProvider`). This layer has no knowledge of Hono, AI SDK, the Todoist client, or MCP implementations.                                                                           | Entities                                 |
...
| `SessionRepository` | `use-cases/ports/session-repository.ts` | Outbound                                            | Loads the current Session, creates new Sessions on `/new`, and appends ConversationMessage entries. Abstracts the `state.json` + per-Session JSONL persistence layout so use-cases remain dependency-free.                                                                                                                                                                                                                              |
| `ChannelGateway`    | `use-cases/ports/channel-gateway.ts`    | Outbound                                            | Delivers outbound replies (from the `Reply` tool or Slash Command handlers) to the originating Channel conversation via a ConversationRef. Channel implementations (e.g., Telegram) provide the adapter.                                                                                                                                                                                                                                |
...
| Job Worker                          | `Job`                                                                        | Use Case          | Shared execution skeleton for heartbeat- and channel-triggered work; assembles prompts via `PromptAssembler` and invokes `ShrimpAgent` **once**; concrete variants are `HeartbeatJob` and `ChannelJob`.                                                                                                                                                                                                                                        | `BoardRepository`, `ShrimpAgent`, `ToolProvider`, `PromptAssembler`                                            |
| Job Worker variant (Heartbeat path) | `HeartbeatJob`                                                               | Use Case          | Selects one Todoist task via `TaskSelector`, promotes Backlog→InProgress, retrieves comments, assembles prompts, and invokes `ShrimpAgent` with empty history.                                                                                                                                                                                                                                                                                 | `BoardRepository`, `ShrimpAgent`, `ToolProvider`, `TaskSelector`, `PromptAssembler`                            |
| Job Worker variant (Channel path)   | `ChannelJob`                                                                 | Use Case          | Loads or creates the current Session via `SessionRepository`, appends the incoming `ChannelMessage`, assembles prompts, invokes `ShrimpAgent` with Session history, and appends returned `ConversationMessage` entries.                                                                                                                                                                                                                        | `SessionRepository`, `ShrimpAgent`, `ToolProvider`, `PromptAssembler`, `ChannelGateway`                        |
| `/new` command handler              | `StartNewSession`                                                            | Use Case          | Creates a new Session and updates the `SessionRepository` current pointer; invoked by the Channel adapter when `/new` arrives.                                                                                                                                                                                                                                                                                                                 | `SessionRepository`                                                                                            |
| Channel (Telegram)                  | `TelegramChannel`                                                            | Infrastructure    | Implements `ChannelGateway`; receives Telegram webhook callbacks (validates secret), normalizes updates into `ChannelMessage` or Slash Command events, and sends replies back through the Telegram Bot API.                                                                                                                                                                                                                                    | Telegram Bot API client                                                                                        |
| Session persistence                 | `JsonlSessionRepository`                                                     | Infrastructure    | Implements `SessionRepository` via `state.json` plus per-Session JSONL files under `SHRIMP_STATE_DIR`; updates the current pointer atomically on `/new`; discards and starts fresh on JSONL corruption.                                                                                                                                                                                                                                        | filesystem                                                                                                     |
| Built-in Reply tool                 | `ReplyTool`                                                                  | Interface Adapter | Inbound adapter from the agent loop; sends the agent's reply text through `ChannelGateway` using the Job's `ConversationRef`; returns a no-op result when `ConversationRef` is null (HeartbeatJob).                                                                                                                                                                                                                                            | `ChannelGateway`                                                                                               |
...
| Todoist API failure (§Todoist Integration API failure handling)                       | Use Case (`Job` top-level catch)                           | Fail-Open Recovery: release the slot, log, let the next Heartbeat retry                                                                                                                           |
| AI SDK / MCP runtime error (§Shrimp Agent Error)                                      | Use Case (`Job` top-level catch)                           | Same as above                                                                                                                                                                                     |
| Individual tool call failure during the agent loop (e.g. Post Comment, Move Task)     | Interface Adapter (the specific tool)                      | The tool returns the error to the agent via the AI SDK tool result so the model can react, retry, or summarize the failure. Only if the entire loop throws does `Job`'s top-level catch take over |
| Board missing required sections (§Prerequisites)                                      | Use Case (`Job` selection phase)                           | Job ends immediately with no side effects                                                                                                                                                         |
| No actionable tasks (§Task selection rule 4)                                          | Use Case (`Job` selection phase)                           | Job ends immediately                                                                                                                                                                              |
