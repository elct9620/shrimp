...
| **Use Cases** | Application orchestration: `MainAgent` (the thin Processing Cycle entry point — selects a task, prepares prompt context, invokes the agent loop once), `PromptAssembler`, and the **port interfaces** for every external dependency (`BoardRepository`, `AgentLoop`, `TaskQueue`, `ToolProvider`). This layer has no knowledge of Hono, AI SDK, the Todoist client, or MCP implementations. | Entities |
...
src/
  ...
  use-cases/
    ports/
      ...
      agent-loop.ts             # Black-box agent loop invocation (outbound)
      task-queue.ts             # Concurrency gate abstraction (inbound, used by HTTP)
      tool-provider.ts          # Merged tool-list provider (outbound)
    main-agent.ts               # Thin Processing Cycle entry point (SPEC §Main Agent)
  ...
  infrastructure/
    ...
    ai/
      ...
      ai-sdk-agent-loop.ts      # AgentLoop implementation (AI SDK tool loop)
    mcp/
...
The four components defined in `SPEC.md` §Architecture Overview are abstracted in code via four ports:
...
| `AgentLoop` | `use-cases/ports/agent-loop.ts` | Outbound | A single black-box invocation of the agent's tool-calling loop. The caller hands over prompts and the full tool set; the adapter runs the loop internally (AI SDK or equivalent) and returns the model's final state when the loop terminates. Shrimp does not control iterations from outside. |
...
| `ToolProvider` | `use-cases/ports/tool-provider.ts` | Outbound | Supplies the currently available tool set (built-in Todoist tools + MCP tools). Consumed by the `AgentLoop` implementation. |

Every port has at least one infrastructure implementation and can be substituted with a fake during testing.

## 7. Component Contracts

SPEC.md §Architecture Overview lists four components: HTTP Layer, Task Queue, Main Agent, and Tool Layer. This section maps those components to concrete modules.
...
| HTTP Layer | `HeartbeatRoute` | Interface Adapter | Accepts `POST /heartbeat`, delegates to `TaskQueue`, responds `202 Accepted` | `TaskQueue`, `MainAgent` |
...
| Main Agent | `MainAgent` | Use Case | Thin Processing Cycle entry point. Selects a task via `TaskSelector` and `BoardRepository`, prepares prompt context, and invokes `AgentLoop` **once**. Everything the agent does during its loop happens inside that single invocation. | `BoardRepository`, `AgentLoop`, `ToolProvider`, `TaskSelector`, `PromptAssembler` |
| Main Agent (support) | `TaskSelector` | Entities | Pure selection policy from SPEC §Task selection rules | — |
| Main Agent (support) | `PromptAssembler` | Use Case | Builds the system prompt (goal + tool descriptions) and user prompt (task content + comment history) | Entities |
| Main Agent (AI) | `AiSdkAgentLoop` | Infrastructure | Implements the `AgentLoop` port via the AI SDK; wires in tools from `ToolProvider` and runs the agentic loop to completion internally | AI SDK, `ToolProvider` |
...
| Port naming | Named after the role, never the implementation (`BoardRepository` not `TodoistApi`; `AgentLoop` not `OpenAiClient`) |
| SPEC terminology | Class names in code follow SPEC terms first (`MainAgent` not `ToolLoopAgent`; `Board` not `TodoistProject`) |
...
| Todoist API failure (§Todoist Integration API failure handling) | Use Case (`MainAgent` top-level catch) | Fail-Open Recovery: release the slot, log, let the next heartbeat retry |
| AI SDK / MCP runtime error (§Main Agent Error) | Use Case (`MainAgent` top-level catch) | Same as above |
| Individual tool call failure during the agent loop (e.g. Post Comment, Move Task) | Interface Adapter (the specific tool) | The tool returns the error to the agent via the AI SDK tool result so the model can react, retry, or summarize the failure. Only if the entire loop throws does `MainAgent`'s top-level catch take over |
| Board missing required sections (§Prerequisites) | Use Case (`MainAgent` selection phase) | Cycle ends immediately with no side effects |
| No actionable tasks (§Task selection rule 4) | Use Case (`MainAgent` selection phase) | Cycle ends immediately |
...
### D2. Main Agent is a thin entry point; the agent loop is a black box
**Decision**: SPEC's "Main Agent" is realized in code as a thin `MainAgent` use case that invokes the `AgentLoop` port **exactly once** per Processing Cycle. Everything the model does — calling tools, deciding when the task is done, iterating — happens inside that single adapter call. Shrimp does not interject between tool calls and does not re-enter the loop.

**Rationale**: The AI SDK (and most agent libraries in the same shape) exposes the tool-calling loop as a single function: you pass prompts and a tool set, it runs to completion internally. Trying to model the six SPEC steps as a step-by-step orchestration in our use case layer would fight the library and make the code diverge from what's actually possible at runtime. The honest framing is: the loop is an adapter we invoke, not a state machine we drive.

**Consequence**: `MainAgent` stays small (select a task, prepare prompt context, invoke once) and the real work lives inside the tools the agent can call. The use case becomes easy to unit-test with a fake `AgentLoop` that simply records what it was given. Swapping AI providers means replacing the `AgentLoop` implementation; the use case is untouched.
