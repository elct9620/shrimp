...
| Main Agent                 | `AiSdkMainAgent`                                                     | Infrastructure    | Implements the `MainAgent` port via the AI SDK; wires in tools from `ToolProvider` and runs the agentic loop to completion internally; annotates the `shrimp.main-agent` span with agent-level gen_ai attrs (`invoke_agent`, `agent.id`, `agent.version`, `provider.name`, `conversation.id`, `error.type`, plus input/output messages)                                                                                                        | AI SDK, `ToolProvider`                                                                                         |
